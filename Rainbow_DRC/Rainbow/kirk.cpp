...
double	Delay_Ratio = 0., Delay_Ratio_BP = 0., Hs = 0., Ls = 0., Lss = 0., Old_Lss = 0., Old_Ls = 0., Ab = 0., Rot_Ang = 0., Old_Rot_Ang = 0.;
...
int		Avg_CNT = 1; 
double RHR_Compen_Ang = 0., LHR_Compen_Ang = 0., RAR_Compen_Ang = 0., LAR_Compen_Ang = 0.;  // 2013_08_02
...
double HeelToe_RAP = 0., HeelToe_LAP = 0.,Old_HeelToe_RAP = 0., Old_HeelToe_LAP = 0., Temp_HeelToe = 0.; // 2013_08_30
...
extern char land_ok_LF, land_ok_RF;
extern unsigned int land_ok_cnt_LF, land_ok_cnt_RF;
// external variables //
...
void init_kirk(void)
{
	...
	// Kirk Init //
	// ?? ???? ?? //
		  Ts = 150;   // [1 sec] Swing Time
		  Ts_BP = 140;//150;   // [0.7 sec] Swing Time
		  Delay_Ratio = 0.1; 
		  Delay_Ratio_BP = 0.01; 
		  Td = (int)(Ts*Delay_Ratio); // [0.1 sec] Delay Time (??, ????)
		  Td_BP = (int)(Ts_BP*Delay_Ratio_BP); // [0.1 sec] Delay Time (??, ????)
	...
	//---
	Del_PC_X_DSP_XZMP_CON = 0.f;
	Del_PC_Y_DSP_YZMP_CON = 0.f;
	Del_PC_X_SSP_XZMP_CON = 0.f;
	Del_PC_Y_SSP_YZMP_CON = 0.f;
	Old_Del_PC_X_DSP_XZMP_CON = 0.; 
	Old_Del_PC_Y_DSP_YZMP_CON = 0.;
	Old_Del_PC_X_SSP_XZMP_CON = 0.; 
	Old_Del_PC_Y_SSP_YZMP_CON = 0.;
	Old_Del_PC_X_SSP_XZMP_CON_2 = 0.; 
	Old_Del_PC_Y_SSP_YZMP_CON_2 = 0.;
	Ref_CON_SUM_X_ZMP_ERR = 0.;
	Ref_CON_SUM_Y_ZMP_ERR = 0.;
	SUM_X_ZMP_ERR = 0.;
	SUM_Y_ZMP_ERR = 0.;
	SUM_RMS_Y_ZMP = 0.;
	CNT_SUM_ZMP_ERR = 0;
							...
							double test_var[10])
{
	...
	static int		profState_R1_Z_up = 0, profState_R1_Z_up_2 = 0, profState_R2_Z_up = 0, profState_R3_Z_up = 0, profState_L1_Z_up = 0, profState_L1_Z_up_2 = 0, profState_L2_Z_up = 0, profState_L3_Z_up = 0,  // 2013_08_30
					profState_R1_Z_dn = 0, profState_R1_Z_dn_2 = 0, profState_R2_Z_dn = 0, profState_R3_Z_dn = 0, profState_L1_Z_dn = 0, profState_L1_Z_dn_2 = 0, profState_L2_Z_dn = 0, profState_L3_Z_dn = 0;  // 2013_08_30
	...
	static double	profResult_R1_Z_up = 0, profResult_R1_Z_up_2 = 0, profResult_R2_Z_up = 0, profResult_R3_Z_up = 0, profResult_L1_Z_up = 0, profResult_L1_Z_up_2 = 0, profResult_L2_Z_up = 0, profResult_L3_Z_up = 0,   // 2013_08_30
					profResult_R1_Z_dn = 0, profResult_R1_Z_dn_2 = 0, profResult_R2_Z_dn = 0, profResult_R3_Z_dn = 0, profResult_L1_Z_dn = 0, profResult_L1_Z_dn_2 = 0, profResult_L2_Z_dn = 0, profResult_L3_Z_dn = 0;   // 2013_08_30
	...
	if(First_Flag_2 == 1) //by Inhyeok
	{
		next_start_time_R1 = Ts/2 + Td;
		next_start_time_L1 = Ts/2 + Td;
		next_start_time_BC = (Ts/2+Td);
		next_start_time_BC_2 = (Ts/2+Td);
		next_start_time_BC_3 = (Ts/2+Td);
		next_start_time_BC_th = (Ts/2+Td);
		//printf("\n >>>>>>>>>>>>>>> In %ld \n",next_start_time_R1);
		First_Flag_2 = 0;
	}
...
//	printf("%d	%d	%d	%d	%d	%d \n",Early_Landing_Flag[0],Early_Landing_Flag[1],Early_Landing_Flag[2],Early_Landing_Flag[3],Early_Landing_Flag[4],Early_Landing_Flag[5]); // 추가 2012.09.17
	...
	if(LF_stop_flag != 2 || profState_R1_Z_dn == 2){
		...
		prof_delay_cos_delay(&profState_R1_Z_up_2,&profResult_R1_Z_up_2,local_Index,next_start_time_R1 + 0,Ts/2,0,Ts/10,Ts/10);    // 2013_08_30
		prof_delay_cos_delay(&profState_R1_Z_dn_2,&profResult_R1_Z_dn_2,local_Index,next_start_time_R1 + Ts/2,Ts/2,0,0,Ts/10); 	 // 2013_08_30 
	...
	if(RF_stop_flag != 2 || profState_L1_Z_dn == 2){
		...
		prof_delay_cos_delay(&profState_L1_Z_up_2,&profResult_L1_Z_up_2,local_Index,next_start_time_L1 + Ts+Old_Td_LF + T_off,Ts/2,0,Ts/10,1*Ts/10);	   // 2013_08_30
		prof_delay_cos_delay(&profState_L1_Z_dn_2,&profResult_L1_Z_dn_2,local_Index,next_start_time_L1 + Ts+Old_Td_LF + T_off + Ts/2,Ts/2,0,0,Ts/10);  // 2013_08_30
	...
	// 발목 Heel and toe motion 추가 //  2013_08_30
	if(Walking_stage[LF_1] != 0){
		if(Old_Ls_LF > 0){
			if(LF_stop_flag != 2) HeelToe_LAP = Foot_Up_AP_Deg*profResult_L1_Z_up_2 + (-Foot_Up_AP_Deg + Foot_Dn_AP_Deg)*profResult_L1_Z_dn_2;
			else HeelToe_LAP = Foot_Up_AP_Deg*profResult_L1_Z_up_2 + (-Foot_Up_AP_Deg)*profResult_L1_Z_dn_2;
			Old_HeelToe_LAP = HeelToe_LAP;
		}
		else if(Old_Ls_LF < 0){
			if(LF_stop_flag != 2) HeelToe_LAP = 0.5*Foot_Up_AP_Deg*profResult_L1_Z_up_2 + (-0.5*Foot_Up_AP_Deg + 0.5*Foot_Dn_AP_Deg)*profResult_L1_Z_dn_2;
			else HeelToe_LAP = 0.5*Foot_Up_AP_Deg*profResult_L1_Z_up_2 + (-0.5*Foot_Up_AP_Deg)*profResult_L1_Z_dn_2;
			Old_HeelToe_LAP = HeelToe_LAP;
		}
		else {HeelToe_LAP = 0.; Old_HeelToe_LAP = 0.;}
		
	}
	else{
		HeelToe_LAP = Old_HeelToe_LAP - Old_HeelToe_LAP*profResult_R1_Z_up_2;
		if(profResult_R1_Z_up == 1.0) Old_HeelToe_LAP = 0.;
	}

	if(Walking_stage[RF_1] != 0){
		if(Old_Ls_RF > 0){
			if(RF_stop_flag != 2) HeelToe_RAP = Foot_Up_AP_Deg*profResult_R1_Z_up_2 + (-Foot_Up_AP_Deg + Foot_Dn_AP_Deg)*profResult_R1_Z_dn_2;
			else  HeelToe_RAP = Foot_Up_AP_Deg*profResult_R1_Z_up_2 + (-Foot_Up_AP_Deg)*profResult_R1_Z_dn_2;
			Old_HeelToe_RAP = HeelToe_RAP;
		}
		else if(Old_Ls_RF < 0){
			if(RF_stop_flag != 2) HeelToe_RAP = 0.5*Foot_Up_AP_Deg*profResult_R1_Z_up_2 + (-0.5*Foot_Up_AP_Deg + 0.5*Foot_Dn_AP_Deg)*profResult_R1_Z_dn_2;
			else  HeelToe_RAP = 0.5*Foot_Up_AP_Deg*profResult_R1_Z_up_2 + (-0.5*Foot_Up_AP_Deg)*profResult_R1_Z_dn_2;
			Old_HeelToe_RAP = HeelToe_RAP;
		}
		else {HeelToe_RAP = 0.0; Old_HeelToe_RAP = 0.;}

	}
	else{
		HeelToe_RAP = Old_HeelToe_RAP - Old_HeelToe_RAP*profResult_L1_Z_up_2;
		if(profResult_L1_Z_up == 1.0) Old_HeelToe_RAP = 0.;
	}
	...
	if(LF_start_flag){
		...
		Temp_HeelToe = HeelToe_LAP;   //2013_08_30
		HeelToe_LAP = HeelToe_RAP;
		HeelToe_RAP = Temp_HeelToe;
...
//	printf("%d	%d	%d	%d	%d	%d \n",Early_Landing_Flag[0],Early_Landing_Flag[1],Early_Landing_Flag[2],Early_Landing_Flag[3],Early_Landing_Flag[4],Early_Landing_Flag[5]); // ?? 2012.09.17
	...
	// Ankle Roll 관절 처짐 보상 각도 궤적 생성 //  2013_08_12
	if(LF_start_flag == 1){
		if(Del_BC_Y > 0) RAR_Compen_Ang = -1.5*Del_BC_Y/40.;
		else RAR_Compen_Ang = 0.;

		if(Del_BC_Y < 0) LAR_Compen_Ang = -1.5*Del_BC_Y/40.;
		else LAR_Compen_Ang = 0.;
	}
	else{
		if(Del_BC_Y > 0) LAR_Compen_Ang = 1.5*Del_BC_Y/40.;
		else LAR_Compen_Ang = 0.;
	...
	if(WP_Flow_Control_Flag != 1){
		...
		if(Go_flag == 0 && local_Index == 0)  { // ?? ???? go flag ? 0 ?? local_index = 0, Step_CNT = 0
			...
			*Step_CNT = 0; BC_SWAY_CNT = 0;
			LF_stop_flag = 0;
			RF_stop_flag = 0;
			RAR_Compen_Ang = 0.;   //2013_08_12
			LAR_Compen_Ang = 0.;   //2013_08_12
			BC_SWAY_Stop_Flag = 0;
			next_start_time_R1 = (Ts/2+Td);
...
void readZMP(double ORF_x, double ORF_y, double OLF_x, double OLF_y, double ORW_x, double ORW_y, double OLW_x, double OLW_y)
{
	...
	if((FTSensor[RFFT].Fz + FTSensor[LFFT].Fz + FTSensor[RWFT].Fz + FTSensor[LWFT].Fz) > 50.){
		...
		X_ZMP_LF = (1. - 2.*PI*2.0*Del_T)*Old_X_ZMP_LF + 2.*PI*2.0*Del_T*X_ZMP;
		Old_X_ZMP_LF = X_ZMP_LF;

		Y_ZMP_LF = (1. - 2.*PI*2.0*Del_T)*Old_Y_ZMP_LF + 2.*PI*2.0*Del_T*Y_ZMP;
		Old_Y_ZMP_LF = Y_ZMP_LF;
    }
...
double P4_XZMP_INT_CON_E(double Ref_ZMP, double u, double ZMP, int zero)
{
	...
	const double A[2][2] = {{0.000000000000, 1.000000000000}, {-595.395118927061, -2.355747368821}};
	const double B[2] = {0.000000000000, 1484.221340007616};
	const double C[2] = {10.970788158523, 0.041757243039};
	const double D = -26.308844504851;	
	const double Kg[2] = {-0.352632794597, 0.011613061397};
	const double Og[2] = {1.780770523256, -45.314379031536};
	const double ki = 0.174545899064;
...
*/
	...
	const double A[2][2] = {{0.000000000000, 1.000000000000}, {-477.621687462925, -2.355747368821}};
	const double B[2] = {0.000000000000, 1201.791048726716};
	const double C[2] = {8.883173049056, 0.041757243039};
	const double D = -21.302573259127;
	const double Kg[2] = {-0.337505998146, 0.014342221609};
	const double Og[2] = {2.187213564608, -42.750522509232};
	const double ki = 0.215565549832;
...
const double C[2] = {10.970788158523, 0.041757243039};
const double D = -26.308844504851;
const double Kg[2] = {-0.384299230547, 0.005150345454};
const double Og[2] = {1.780770523256, -45.314379031536};
...
const double C[2] = {8.883173049056, 0.041757243039};
const double D = -21.302573259127;
const double Kg[2] = {-0.380566728257, 0.005528625495};
const double Og[2] = {2.187213564608, -42.750522509232};
...
double P3H_XZMP_CON_E(double u, double ZMP, int zero)   // 손을 들었을때
{	
/*
...
const double A[2][2] = {{0.000000000000, 1.000000000000}, {-197.139503033525, -1.217815421806}};
const double B[2] = {0.000000000000, 529.171901414006};
const double C[2] = {3.911433337716, 0.021586616298};
const double D = -9.379936061670;
const double Kg[2] = {-0.355516803766, 0.009037109804};
const double Og[2] = {3.479399872646, -38.322637855937};
...
double P3H_YZMP_CON_E(double u, double ZMP, int zero) // 손을 들었을 때 
{	
...
const double A[2][2] = {{0.000000000000, 1.000000000000}, {-203.917446303499, -0.676564123225}};
const double B[2] = {0.000000000000, 545.425962013463};
const double C[2] = {4.031577045899, 0.011992564610};
const double D = -9.668050469782;

const double Kg[2] = {-0.357349044376, 0.009760143901};
const double Og[2] = {2.445116719065, -44.547649291761};
...
double P3F_XZMP_CON_E(double u, double ZMP, int zero)    // 발을 들었을 때
{	
/*
...
const double A[2][2] = {{0.000000000000, 1.000000000000}, {-273.396243797489, -2.029692369676}};
const double B[2] = {0.000000000000, 712.041783341977};
const double C[2] = {5.263136537992, 0.035977693830};
const double D = -12.621430546743;
const double Kg[2] = {-0.361476320377, 0.008384771470};
const double Og[2] = {2.929541582511, -40.254656649347};
...
double P3F_YZMP_CON_E(double u, double ZMP, int zero)      // 발을 들었을 때
{	
/*
...
const double A[2][2] = {{0.000000000000, 1.000000000000}, {-304.617419786709, -2.029692369676}};
const double B[2] = {0.000000000000, 786.912708975597};
const double C[2] = {5.816553364861, 0.035977693830};
const double D = -13.948569220291;
const double Kg[2] = {-0.366759129056, 0.007587001153};
const double Og[2] = {2.823375034087, -40.358449080377};
...
double RLAR_TORQ_CON(double Torq, int zero)
{
	...
	const float adm[4] = {-0.002197901385, -13.156455143489, 0.000156067084, 0.934204599931};
	const float bdm[2] = {0.000156067084, 0.000000780491};
...
double RLAR_TORQ_CON_BP(double Torq, int zero)
{
	static double x1new,x2new, x1=0., x2=0.;
...
double RLAP_TORQ_CON_BP(double Torq, int zero)
{
	static double x1new,x2new, x1=0., x2=0.;
...
double P3F_Y_VIB_CON_E_RF(double u, double Ang_Vel, int zero) // 오일러 방식 //
{


	const double A[2][2] = {{0.000000000000, 1.000000000000}, {-1178.847311187478, 0.000000000000}};
	const double B[2] = {0.000000000000, 1135.247311187478};
	const double C[2] = {0.000000000000, 1.000000000000};
	const double D = 0.000000000000;
	const double Kg[2] = {-0.950310386606, 0.017617306646};
	const double Og[2] = {-1.120724182237, 100.000000000000};
...
double P3F_Y_VIB_CON_E_LF(double u, double Ang_Vel, int zero) // 오일러 방식 //
{
...
void ZMP_CON_MANAGER(double R1_Load_N, double R2_Load_N, double L1_Load_N, double L2_Load_N, unsigned char R1_Wstage, unsigned char L1_Wstage, int zero) // update
{
	...
	Del_YZMP_CON_P3H = k3H(Number_Support_LP, R1_Wstage, L1_Wstage)*0.5*P3H_YZMP_CON_E(-Del_YZMP_CON,Y_ZMP_LF - 1.*Ref_ZMP[1], 1); 
	if(Ls >= 0) Del_XZMP_CON_P3H = k3H(Number_Support_LP, R1_Wstage, L1_Wstage)*0.8*P3H_XZMP_CON_E(-Del_XZMP_CON,X_ZMP - X_ZMP_OFFSET - 1.*Ref_ZMP[0], 1);
	else Del_XZMP_CON_P3H = k3H(Number_Support_LP, R1_Wstage, L1_Wstage)*0.8*P3H_XZMP_CON_E(-Del_XZMP_CON,X_ZMP - X_ZMP_OFFSET_BWD - 1.*Ref_ZMP[0], 1); 
	Old_Del_YZMP_CON_P3H = Del_YZMP_CON_P3H;
	Old_Del_XZMP_CON_P3H = Del_XZMP_CON_P3H;
	
	Del_YZMP_CON_P3F = k3F(Number_Support_LP,R1_Wstage, L1_Wstage)*0.8*P3F_YZMP_CON_E(-Del_YZMP_CON,Y_ZMP - 1.*Ref_ZMP[1], 1); 
	if(Ls >= 0) Del_XZMP_CON_P3F = k3F(Number_Support_LP,R1_Wstage, L1_Wstage)*0.8*P3F_XZMP_CON_E(-Del_XZMP_CON,X_ZMP - X_ZMP_OFFSET - 1.*Ref_ZMP[0], 1);
	else Del_XZMP_CON_P3F = k3F(Number_Support_LP,R1_Wstage, L1_Wstage)*0.8*P3F_XZMP_CON_E(-Del_XZMP_CON,X_ZMP - X_ZMP_OFFSET_BWD - 1.*Ref_ZMP[0], 1); 
...
void ZMP_CON_MANAGER_BP(double R1_Load_N, double L1_Load_N, unsigned char Land_ROK_Flag, unsigned char Land_LOK_Flag, int zero) // update
{
	...
	Del_XZMP_CON_P2 = k2(Number_Support_LP)*1.0*P2_XZMP_CON_E(-Del_XZMP_CON,X_ZMP - pSharedMemory->xzmp_offset - 1.*Ref_ZMP[0], 1); 
	...
	Del_XZMP_CON_P1 = k1(Number_Support_LP)*1.0*P1_XZMP_CON_E(-Del_XZMP_CON,X_ZMP - pSharedMemory->xzmp_offset - 1.*Ref_ZMP[0], 1);
...
void Kirk_con_test_BP(void)
{
	...
	if(pSharedMemory->P4_ZMP_CON_CNT < 5599 && pSharedMemory->P4_ZMP_CON_Flag) { 
		...
		//Del_XZMP_CON = 1.0*P2_XZMP_CON_E(-Del_XZMP_CON,X_ZMP - pSharedMemory->xzmp_offset, 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));
		//Del_XZMP_CON = 1.0*P2_XZMP_CON(X_ZMP - pSharedMemory->xzmp_offset, 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));
		//Del_XZMP_CON = -1.0*P2_XCOM_ZMP_CON_E(0., X_ZMP - pSharedMemory->xzmp_offset, 0., 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));

		//Del_YZMP_CON = 1.*0.6*P2_YZMP_INT_CON_E(0.,Del_YZMP_CON,Y_ZMP, 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));  
		//Del_XZMP_CON = 1.*0.6*P2_XZMP_INT_CON_E(pSharedMemory->xzmp_offset,Del_XZMP_CON,X_ZMP, 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));
	...
	if(pSharedMemory->P4_ZMP_CON_CNT < 5600) pSharedMemory->P4_ZMP_CON_CNT++;
		...
		P1_XZMP_CON_E(0,0,0); P1_YZMP_CON_E(0,0,0);
...
void Kirk_con_test(void)
{
	...
	if(pSharedMemory->P4_ZMP_CON_CNT < 5599 && pSharedMemory->P4_ZMP_CON_Flag) { 
		...
		//적분식//
		//Del_YZMP_CON = 0.85*P4_YZMP_CON_E(-Del_YZMP_CON,Y_ZMP, 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));  
		//Del_XZMP_CON = 0.75*P4_XZMP_CON_E(-Del_XZMP_CON,X_ZMP - X_ZMP_OFFSET, 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));
		Del_YZMP_CON = 0.5*P3H_YZMP_CON_E(-Del_YZMP_CON,Y_ZMP_LF + 70., 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));  
		Del_XZMP_CON = 0.8*P3H_XZMP_CON_E(-Del_XZMP_CON,X_ZMP - 140., 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));
		//Del_YZMP_CON = 0.80*P3F_YZMP_CON_E(-Del_YZMP_CON,Y_ZMP + 115., 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));  
		//Del_XZMP_CON = 0.80*P3F_XZMP_CON_E(-Del_XZMP_CON,X_ZMP - 110, 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));
		//Del_YZMP_CON = 0.*0.6*P4_YZMP_INT_CON_E(0.,Del_YZMP_CON,Y_ZMP, 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));     // 주의 : -Del_YZMP_CON --> Del_YZMP_CON
		//Del_XZMP_CON = 0.*0.6*P4_XZMP_INT_CON_E(X_ZMP_OFFSET,Del_XZMP_CON,X_ZMP, 1)*0.5*(1-cos(PI*(P4_ZMP_CON_CNT_2)/200));  // 주의 : -Del_XZMP_CON --> Del_XZMP_CON
		...
		Del_LHY_VIB_CON = 0.*1.0*R2D*P3F_Y_VIB_CON_E_LF(-D2R*Del_LHY_VIB_CON,0.006*LF_Y_acc_HPF_INT, 1); // [deg]
	...
	_log_temp[0] = X_ZMP;                   // a
	_log_temp[1] = Y_ZMP;                   // b
	_log_temp[2] = Del_XZMP_CON;            // c1
	_log_temp[3] = Del_YZMP_CON;			// c2
	_log_temp[4] = FTSensor[LFFT].dAccRoll;	// c3
	_log_temp[5] = LF_Y_acc_HPF_INT;		// c4
	_log_temp[6] = Del_LHY_VIB_CON;			// c5
	_log_temp[7] = FTSensor[LFFT].dAccPitch;// c6
	_log_temp[8] = LF_X_acc_HPF_INT;		// c7
	_log_temp[9] = Del_RHP_VIB_CON;			// c8
	_log_temp[10] = FTSensor[RFFT].Fz;		// c9
	_log_temp[11] = FTSensor[LFFT].Fz;		// c10
	_log_temp[12] = FTSensor[RWFT].Fz;		// c11
	_log_temp[13] = FTSensor[LWFT].Fz;		// c12
	_log_temp[14] = FTSensor[RFFT].Mx;		// c13
	_log_temp[15] = FTSensor[LFFT].Mx;		// c14
	_log_temp[16] = FTSensor[RWFT].Mx;		// c15
	_log_temp[17] = FTSensor[LWFT].Mx;		// c16
	_log_temp[18] = Y_ZMP_LF;                // c17
	...
	_online_pattern.offset_RAP = (Del_RAP_Init + 0*Del_RLHP_Init)*D2R;
	_online_pattern.offset_RAR = Del_RAR_Init*D2R;

	_online_pattern.offset_LAP = 0*Del_RLHP_Init*D2R;
	_online_pattern.offset_LAR = Del_LAR_Init*D2R;


    _online_pattern.dpRFz = 0.001*Ref_R1[2] + 0.001*Del_R1_Z_PosInit + 0.0*Del_RLFZ_Init;
    _online_pattern.dpLFz = 0.001*Ref_L1[2] - 0.001*Del_RLFZ_Init;//+ 0.001*Del_LEG_Z_VSC[LF1];
    _online_pattern.dpRFy = 0.001*Ref_R1[1] - 0.001*Ref_BC[1] - 0.001*BC_Y_Init - 0.001*BC_Y_CON;  // update
    _online_pattern.dpLFy = 0.001*Ref_L1[1] - 0.001*Ref_BC[1] - 0.001*BC_Y_Init - 0.001*BC_Y_CON; // update
    _online_pattern.dpRFx = 0.001*Ref_R1[0] - 0.001*Ref_BC[0] - 0.001*BC_X_Init - 0.001*BC_X_CON; // update
    ...
    _online_pattern.dpRHz = 0.001*Ref_R2[2] + 0.0*Del_R2_Z_PosInit;//+ 0.001*Del_LEG_Z_VSC[RF2];
    _online_pattern.dpLHz = 0.001*Ref_L2[2] ;//+ 0.001*Del_LEG_Z_VSC[LF2];
    _online_pattern.dpRHy = 0.001*Ref_R2[1] - 0.001*Ref_BC[1] - 0.001*BC_Y_Init - 0.001*BC_Y_CON; // update
    _online_pattern.dpLHy = 0.001*Ref_L2[1] - 0.001*Ref_BC[1] - 0.001*BC_Y_Init - 0.001*BC_Y_CON; // update
    _online_pattern.dpRHx = 0.001*Ref_R2[0] - 0.001*Ref_BC[0] - 0.001*BC_X_Init - 0.001*BC_X_CON; // update
	...
	if(pSharedMemory->P4_ZMP_CON_CNT < 5600) pSharedMemory->P4_ZMP_CON_CNT++;
		...
		P3H_YZMP_CON_E(0,0,0); P3H_XZMP_CON_E(0,0,0);
		P3F_YZMP_CON_E(0,0,0); P3F_XZMP_CON_E(0,0,0);
		P4_XZMP_INT_CON_E(0,0,0,0); P4_YZMP_INT_CON_E(0,0,0,0);
		P2_YZMP_CON_E(0,0,0); P2_XZMP_CON_E(0,0,0); P2_XZMP_CON(0,0);
		SSP_Y_VIB_CON_E_RF(0,0,0); SSP_X_VIB_CON_E_RF(0,0,0); SSP_Z_VIB_CON_E(0,0,0);
		P3F_Y_VIB_CON_E_RF(0,0,0); P3F_Y_VIB_CON_E_LF(0,0,0);
		Old_Del_RHR_VIB_CON = 0.; Del_RHR_VIB_CON = 0.;
		Old_Del_RHP_VIB_CON = 0.; Del_RHP_VIB_CON = 0.;
		Old_Del_LHY_VIB_CON = 0.; Del_LHY_VIB_CON = 0.;
		P2_XCOM_ZMP_CON_E(0,0,0,0);
...
void Kirk_Posture_Init(void)
{
	...
	// ZMP 적분 제어기 //	
	Del_YZMP_CON = 0.6*P4_YZMP_INT_CON_E(0.,Del_YZMP_CON,Y_ZMP, 1)*0.5*(1-cos(PI*(Local_CNT_1)/100));  
	Del_XZMP_CON = 0.6*P4_XZMP_INT_CON_E(X_ZMP_OFFSET,Del_XZMP_CON,X_ZMP, 1)*0.5*(1-cos(PI*(Local_CNT_1)/100));
...
void Kirk_Posture_Init_BP(void)
{
	...
	Del_XZMP_CON = 0.8*P2_XZMP_INT_CON_E(pSharedMemory->xzmp_offset,Del_XZMP_CON,X_ZMP, 1)*0.5*(1-cos(PI*(Local_CNT_1)/100));
	...
	Del_RLFZ_Init =  TR_INT_CON(0.,IMUSensor[CENTERIMU].Roll,1);    // 가속도계에 의한 경사만으로 상체 롤 기울기 교정 //
	...
	pSharedMemory->Del_RAP_Init = Del_RAP_Init;
	pSharedMemory->Del_RAR_Init = Del_RAR_Init;
	pSharedMemory->Del_LAR_Init = Del_LAR_Init;
	pSharedMemory->Del_RLHP_Init = Del_RLHP_Init;	
	pSharedMemory->BC_X_Init = BC_X_Init;
	pSharedMemory->BC_Y_Init = BC_Y_Init;
	pSharedMemory->Del_RLFZ_Init = Del_RLFZ_Init;
...
{
	static char temp_flag = 0;
	static float Del_RAP_Init0 = Del_RAP_Init;
	static float Del_RAR_Init0 = Del_RAR_Init;
	static float Del_LAR_Init0 = Del_LAR_Init;
	static float Del_RLHP_Init0 = Del_RLHP_Init;
	static float BC_X_Init0 = BC_X_Init;
	static float BC_Y_Init0 = BC_Y_Init;
	static float Del_RLFZ_Init0 = Del_RLFZ_Init;
	...
	if(temp_flag==0)
	{
		...
		Del_RAP_Init0 = Del_RAP_Init;
		Del_RAR_Init0 = Del_RAR_Init;
		Del_LAR_Init0 = Del_LAR_Init;
		Del_RLHP_Init0 = Del_RLHP_Init;
		BC_X_Init0 = BC_X_Init;
		BC_Y_Init0 = BC_Y_Init;
		Del_RLFZ_Init0 = Del_RLFZ_Init;
		temp_flag = 1;
	}
	
	Del_RAP_Init = Del_RAP_Init0 + (-Del_RAP_Init0+pSharedMemory->Del_RAP_Init)*0.5f*(1.f-cosf(PI*Local_CNT_1/400));
	Del_RAR_Init = Del_RAR_Init0 + (-Del_RAR_Init0+pSharedMemory->Del_RAR_Init)*0.5f*(1.f-cosf(PI*Local_CNT_1/400));
	Del_LAR_Init = Del_LAR_Init0 + (-Del_LAR_Init0+pSharedMemory->Del_LAR_Init)*0.5f*(1.f-cosf(PI*Local_CNT_1/400));
	Del_RLHP_Init = Del_RLHP_Init0 + (-Del_RLHP_Init0+pSharedMemory->Del_RLHP_Init)*0.5f*(1.f-cosf(PI*Local_CNT_1/400));	
	Old_Del_XZMP_CON = Del_XZMP_CON = BC_X_Init = BC_X_Init0 + (-BC_X_Init0+pSharedMemory->BC_X_Init)*0.5f*(1.f-cosf(PI*Local_CNT_1/400));
	Old_Del_YZMP_CON = Del_YZMP_CON = BC_Y_Init = BC_Y_Init0 + (-BC_Y_Init0+pSharedMemory->BC_Y_Init)*0.5f*(1.f-cosf(PI*Local_CNT_1/400));
	Del_RLFZ_Init = Del_RLFZ_Init0 + (-Del_RLFZ_Init0+pSharedMemory->Del_RLFZ_Init)*0.5f*(1.f-cosf(PI*Local_CNT_1/400));
...
void Go_Home_Posture(void)
{
	...
	else
	{
		...
		TR_INT_CON(0,0,0);
		ZMP_CON_MANAGER_BP(0,0,0,0,0);
		RLAR_TORQ_CON(0,0); RLAP_TORQ_CON(0,0); RLAR_TORQ_CON_BP(0,0); RLAP_TORQ_CON_BP(0,0);
...
void Kirk_online(void)
{
	...
	if(Fz[LF_1] > 70 && LandLOK_Flag == 0) LandLOK_Flag = 1; // 왼발 완전 착지 기준 ( 10kg )

	if(Fz[RF_1] > 70 && LandROK_Flag == 0) LandROK_Flag = 1; // 오른발 완전 착지 기준 ( 10kg )
	...
	// Swing foot 측방향 진동 제어 // 
	if(Walking_stage[RF_1] == 10){

			final_gain_SSP_Y_VIB_CON_2 = 0.5*(1-cos(PI*CNT_final_gain_SSP_Y_VIB_CON_2/10));

			Del_RHY_VIB_CON = 1.0*final_gain_SSP_Y_VIB_CON_2*R2D*P3F_Y_VIB_CON_E_RF(-D2R*Del_RHY_VIB_CON,0.006*RF_Y_acc_HPF_INT, 1); // [deg]
			...
			final_gain_SSP_Y_VIB_CON_4 = 1.0 - 0.5*(1-cos(PI*CNT_FOOT_VIB_CON/(Ts/3)));

			Del_RHY_VIB_CON = 1.0*final_gain_SSP_Y_VIB_CON_4*R2D*P3F_Y_VIB_CON_E_RF(-D2R*Del_RHY_VIB_CON,0.006*RF_Y_acc_HPF_INT, 1); // [deg]
			

			if(CNT_FOOT_VIB_CON < (Ts/3)) CNT_FOOT_VIB_CON++;
			else {P3F_Y_VIB_CON_E_RF(0,0,0);  CNT_final_gain_SSP_Y_VIB_CON_1 = 0; final_gain_SSP_Y_VIB_CON_1 = 0.;}
	...
	if(Walking_stage[LF_1] == 10){
			...
			Del_LHY_VIB_CON = 1.0*final_gain_SSP_Y_VIB_CON_1*R2D*P3F_Y_VIB_CON_E_LF(-D2R*Del_LHY_VIB_CON,0.006*LF_Y_acc_HPF_INT, 1); // [deg]
			...
			final_gain_SSP_Y_VIB_CON_3 = 1.0 - 0.5*(1-cos(PI*CNT_FOOT_VIB_CON/(Ts/3)));

			Del_LHY_VIB_CON = 1.0*final_gain_SSP_Y_VIB_CON_3*R2D*P3F_Y_VIB_CON_E_LF(-D2R*Del_LHY_VIB_CON,0.006*LF_Y_acc_HPF_INT, 1); // [deg]
			
			if(CNT_FOOT_VIB_CON < (Ts/3)) CNT_FOOT_VIB_CON++;
			else {P3F_Y_VIB_CON_E_LF(0,0,0);   CNT_final_gain_SSP_Y_VIB_CON_2 = 0; final_gain_SSP_Y_VIB_CON_2 = 0.;}
	...
	// Early Landing Algorithm // 
     ...
     if(All_Foot_Z_Flag ){
		   All_Foot_Z = Old_All_Foot_Z + (temp_Z_avg - Old_All_Foot_Z)*0.5*(1-cos(PI*All_Foot_Z_CNT/(Ts/2)));

		   if(All_Foot_Z_CNT < (Ts/2)) All_Foot_Z_CNT++;
		   else{
	...
	if(Early_Landing_Flag[0] == 1 && temp_X[0] == 0. && temp_Y[0] == 0. && temp_th[0] == 0.) {temp_X[0] = Ref_R1[0]; temp_Y[0] = Ref_R1[1]; temp_th[0] = Ref_th[0]; EL_Del_xy_CNT[0] = 0;}
		...
		//printf("temp_X[RF1] = %f,	temp_X[LF1] = %f, Old_Ref_R1[0] = %f, EL_Del_xy_CNT[0] = %d\n",temp_X[RF1], temp_X[LF1], Old_Ref_R1[0],EL_Del_xy_CNT[0]);
	...
	if(Early_Landing_Flag[1] == 1 && temp_X[1] == 0. && temp_Y[1] == 0. && temp_th[1] == 0.) {temp_X[1] = Ref_L2[0]; temp_Y[1] = Ref_L2[1]; temp_th[1] = Ref_th[1]; EL_Del_xy_CNT[1] = 0;}
	else if(Early_Landing_Flag[1] == 1 && (temp_X[1] != 0. || temp_Y[1] != 0. || temp_th[1] != 0.)) {
		Ref_L2[0] = temp_X[1]; Ref_L2[1] = temp_Y[1]; Ref_th[1] = temp_th[1];
		Old_Ref_L2[0] = Ref_L2[0]; Old_Ref_L2[1] = Ref_L2[1]; Old_Ref_th[1] = Ref_th[1];
	}
	else if(Early_Landing_Flag[1] == 0 && (temp_X[1] != 0. || temp_Y[1] != 0. || temp_th[1] != 0.)){
		if(temp_X[1] != 0.) Ref_L2[0] = Old_Ref_L2[0] + (Ref_L2[0] - Old_Ref_L2[0])*0.5*(1-cos(PI/60*EL_Del_xy_CNT[1]));
		if(temp_Y[1] != 0.) Ref_L2[1] = Old_Ref_L2[1] + (Ref_L2[1] - Old_Ref_L2[1])*0.5*(1-cos(PI/60*EL_Del_xy_CNT[1]));
		if(temp_th[1] != 0.) Ref_th[1] = Old_Ref_th[1] + (Ref_th[1] - Old_Ref_th[1])*0.5*(1-cos(PI/60*EL_Del_xy_CNT[1]));

		if(EL_Del_xy_CNT[1] < 60 && WP_Flow_Control_Flag == 0) EL_Del_xy_CNT[1]++;
		else if(WP_Flow_Control_Flag == 0){Old_Ref_L2[0] = 0.;temp_X[1] = 0.; Old_Ref_L2[1] = 0.;temp_Y[1] = 0.; Old_Ref_th[1] = 0.; temp_th[1] = 0.;}
	}
	...
	if(Early_Landing_Flag[4] == 1 && temp_X[4] == 0. && temp_Y[4] == 0. && temp_th[4] == 0.) {temp_X[4] = Ref_R2[0]; temp_Y[4] = Ref_R2[1]; temp_th[4] = Ref_th[4]; EL_Del_xy_CNT[4] = 0;}
	else if(Early_Landing_Flag[4] == 1 && (temp_X[4] != 0. || temp_Y[4] != 0. || temp_th[4] != 0.)) {
		Ref_R2[0] = temp_X[4]; Ref_R2[1] = temp_Y[4]; Ref_th[4] = temp_th[4];
		Old_Ref_R2[0] = Ref_R2[0]; Old_Ref_R2[1] = Ref_R2[1]; Old_Ref_th[4] = Ref_th[4];
	}
	else if(Early_Landing_Flag[4] == 0 && (temp_X[4] != 0. || temp_Y[4] != 0. || temp_th[4] != 0.)){
		if(temp_X[4] != 0.) Ref_R2[0] = Old_Ref_R2[0] + (Ref_R2[0] - Old_Ref_R2[0])*0.5*(1-cos(PI/60*EL_Del_xy_CNT[4]));
		if(temp_Y[4] != 0.) Ref_R2[1] = Old_Ref_R2[1] + (Ref_R2[1] - Old_Ref_R2[1])*0.5*(1-cos(PI/60*EL_Del_xy_CNT[4]));
		if(temp_th[4] != 0.) Ref_th[4] = Old_Ref_th[4] + (Ref_th[4] - Old_Ref_th[4])*0.5*(1-cos(PI/60*EL_Del_xy_CNT[4]));

		if(EL_Del_xy_CNT[4] < 60 && WP_Flow_Control_Flag == 0) EL_Del_xy_CNT[4]++;
		else if(WP_Flow_Control_Flag == 0){Old_Ref_R2[0] = 0.;temp_X[4] = 0.;Old_Ref_R2[1] = 0.;temp_Y[4] = 0.; Old_Ref_th[4] = 0.; temp_th[4] = 0.;}
	}
	...
	for( i = 0; i<6; i++){
			...
			if((i == RF_1 || i == LF_1) && Late_Landing_Flag[i] == 1 && Fz[i] < EARLY_LAND_THRES_1 && LL_Del_z[i] < LATE_LAND_MAX_DEPTH && Walking_stage[i] == 0){
						...
						LL_Del_z[i] = Old_LL_Del_z[i] - Old_LL_Del_z[i]*0.5*(1-cos(PI*LL_Del_z_CNT[i]/(Ts/2)));

						if(LL_Del_z_CNT[i] < (Ts/2) && WP_Flow_Control_Flag == 0) LL_Del_z_CNT[i]++;
	...
	if(0){ //no control
		...
		Del_LAR_RLAR_TORQ_CON = 0.; Del_LAP_RLAP_TORQ_CON = 0., Del_RAR_RLAR_TORQ_CON = 0.; Del_RAP_RLAP_TORQ_CON = 0.;
		Del_RHY_VIB_CON = 0.; Del_LHY_VIB_CON = 0.;
	}
	...
	_online_pattern.offset_LAR = (Del_LAR_Init + Del_LAR_RLAR_TORQ_CON + LAR_Compen_Ang)*D2R;
	_online_pattern.offset_LAP = (Del_LAP_RLAP_TORQ_CON )*D2R;

    _online_pattern.offset_RAR = (Del_RAR_Init + Del_RAR_RLAR_TORQ_CON + RAR_Compen_Ang)*D2R;
	_online_pattern.offset_RAP = (Del_RAP_Init + Del_RAP_RLAP_TORQ_CON )*D2R;
	...
	_log_temp[0] = 0.001*X_ZMP;         // a
	_log_temp[1] = 0.001*Y_ZMP;         // b
	_log_temp[2] = 0.001*Ref_BC[0];     // c1
	_log_temp[3] = 0.001*Ref_BC[1];     // c2
	_log_temp[4] = 0.001*BC_X_CON;      // c3
	_log_temp[5] = 0.001*BC_Y_CON;      // c4
	_log_temp[6] = 0.001*Ref_ZMP[0];    // c5
	_log_temp[7] = 0.001*Ref_ZMP[1];    // c6
	_log_temp[8] = 0.001*Ref_R1[2];     // c7
	_log_temp[9] = 0.001*Ref_L1[2];     // c8
	_log_temp[10] = 0.001*Ref_R2[2];    // c9
	_log_temp[11] = 0.001*Ref_L2[2];    // c10
	_log_temp[12] = Del_LAR_RLAR_TORQ_CON;  //c11
	_log_temp[13] = Del_LAP_RLAP_TORQ_CON;  //c12
	_log_temp[14] = FTSensor[RFFT].Fz;	//c13
	_log_temp[15] = FTSensor[LFFT].Fz;	//c14
	_log_temp[16] = FTSensor[RWFT].Fz;	//c15
	_log_temp[17] = FTSensor[LWFT].Fz;	//c16
...
{
	//------------- controller envelop gain
	static float env_EL = 1.f; 
	static float env_ZMP = 1.f;
	static float env_vib = 1.f;
	static float env_upright = 1.f;
	static unsigned int env_ZMP_count = 0;
	static unsigned int env_vib_count = 0;
	static unsigned int env_upright_count = 0;
	float ftemp;

	if(pSharedMemory->ON_EL_flag == 0)
	{
		env_EL = 0.f;
		pSharedMemory->ON_EL_flag = -1;
	}
	else if(pSharedMemory->ON_EL_flag == 1)
	{
		env_EL = 1.f;
		pSharedMemory->ON_EL_flag = -1;
	}

	if(pSharedMemory->ON_ZMP_flag == 0)
	{
		env_ZMP = 0.5f*(1.f + cosf(PI*(float)env_ZMP_count/200.f));
		env_ZMP_count++;
		if(env_ZMP_count > 200)
		{
			env_ZMP = 0.f;
			env_ZMP_count = 0;
	...
	else if(pSharedMemory->ON_ZMP_flag == 1)
	{
		env_ZMP = 0.5f*(1.f - cosf(PI*(float)env_ZMP_count/200.f));
		env_ZMP_count++;
		if(env_ZMP_count > 200)
	...
	if(pSharedMemory->ON_vib_flag == 0)
	{
		env_vib = 0.5f*(1.f + cosf(PI*(float)env_vib_count/200.f));
		env_vib_count++;
		if(env_vib_count > 200)
		{
			env_vib = 0.f;
			env_vib_count = 0;
	...
	else if(pSharedMemory->ON_vib_flag == 1)
	{
		env_vib = 0.5f*(1.f - cosf(PI*(float)env_vib_count/200.f));
		env_vib_count++;
		if(env_vib_count > 200)
	...
	if(pSharedMemory->ON_upright_flag == 0)
	{
		env_upright = 0.5f*(1.f + cosf(PI*(float)env_upright_count/200.f));
		env_upright_count++;
		if(env_upright_count > 200)
		{
			env_upright = 0.f;
			env_upright_count = 0;
	...
	else if(pSharedMemory->ON_upright_flag == 1)
	{
		env_upright = 0.5f*(1.f - cosf(PI*(float)env_upright_count/200.f));
		env_upright_count++;
		if(env_upright_count > 200)
	...
	if(env_vib == 0.f)
	{
		SSP_Y_VIB_CON_E_RF(0,0,0);
		SSP_X_VIB_CON_E_RF(0,0,0);
		SSP_Y_VIB_CON_E_LF(0,0,0);
		SSP_X_VIB_CON_E_LF(0,0,0);
		Del_RHR_VIB_CON = 0.f;
		Del_RHP_VIB_CON = 0.f;
		Del_LHR_VIB_CON = 0.f;
		Del_LHP_VIB_CON = 0.f;
	...
	if(env_upright == 0.f)
	{
		TP_INT_CON_WALK(0,0,0);
		TR_INT_CON_WALK(0,0,0);
		Del_RLHP_Posture_Walking = 0.f;
		Del_RLAR_Posture_Walking = 0.f;
	}
	//----------------
	...
	/* 착지 Threshold 설정 (로봇 총 무게 : 42 kgf ) */
	if(Fz[LF_1] > 100 && LandLOK_Flag == 0) LandLOK_Flag = 1; // 왼발 완전 착지 기준 ( 10kg )

	if(Fz[RF_1] > 100 && LandROK_Flag == 0) LandROK_Flag = 1; // 오른발 완전 착지 기준 ( 10kg )
	...
	if(Fz[LF_1] > 150 && land_ok_LF==0)  // Inhyeok
		land_ok_cnt_LF++;
	
	if(land_ok_cnt_LF>10 && land_ok_LF==0)
	{
		land_ok_LF = 1;
		land_ok_cnt_LF = 0;
	}

	if(Fz[RF_1] > 150 && land_ok_RF==0)  // Inhyeok
		land_ok_cnt_RF++;
	
	if(land_ok_cnt_RF>10 && land_ok_RF==0)
	{
		land_ok_RF = 1;
		land_ok_cnt_RF = 0;
	}
    ...
    if(Change_flag == 1 || pSharedMemory->First_flag == 1){
		...
		Lss = pSharedMemory->Global_Lss;
		Rot_Ang = pSharedMemory->Global_Rot_Ang;
		Delay_Ratio_BP = pSharedMemory->Global_Delay_Ratio;
		Td_BP = (int)(Ts_BP*Delay_Ratio_BP);
		Hs = pSharedMemory->Global_Hs;
		...
		if(Delay_Ratio_BP < 0) {Delay_Ratio_BP = 0. ; pSharedMemory->Global_Delay_Ratio = 0.;}
	...
	if(Walking_stage[RF_1] == 0 && Walking_stage[LF_1] == 0){ 
		...
		Del_PC_X_DSP_XZMP_CON = final_gain_DSP_ZMP_CON*P2_XZMP_CON_E(-Del_PC_X_DSP_XZMP_CON,X_ZMP - pSharedMemory->xzmp_offset - Ref_ZMP[0], 1);
	...
	if((Walking_stage[RF_1] == 10  && Walking_stage[LF_1] == 0) || Walking_stage[RF_1] == 0  && Walking_stage[LF_1] == 10) {
		...
		Del_PC_X_SSP_XZMP_CON = Old_Del_PC_X_SSP_XZMP_CON + final_gain_SSP_ZMP_CON*(-Old_Del_PC_X_SSP_XZMP_CON + 1.*P1_XZMP_CON_E(-Del_PC_X_SSP_XZMP_CON, X_ZMP - pSharedMemory->xzmp_offset - Ref_ZMP[0], 1));
		Del_PC_Y_SSP_YZMP_CON = Old_Del_PC_Y_SSP_YZMP_CON + final_gain_SSP_ZMP_CON*(-Old_Del_PC_Y_SSP_YZMP_CON + 1.*P1_YZMP_CON_E(-Del_PC_Y_SSP_YZMP_CON, Y_ZMP - Ref_ZMP[1], 1));
		...
		*/		
		...
		Del_PC_X_SSP_XZMP_CON = 1.*P1_XZMP_CON_E(-Del_PC_X_SSP_XZMP_CON, X_ZMP - pSharedMemory->xzmp_offset - Ref_ZMP[0], 1);
		Del_PC_Y_SSP_YZMP_CON = 1.*P1_YZMP_CON_E(-Del_PC_Y_SSP_YZMP_CON, Y_ZMP - Ref_ZMP[1], 1);
	...
	if(Step_CNT >= 2 && Real_Stop_Flag == 0){   // 첫번째 스텝은 비대칭이라 무시. 두 번째 스텝부터 반영함.
		if(Step_CNT >= 4 && Old_Step_CNT%2 == 1 && Step_CNT%2 == 0 ){
			...
			CNT_CON_SUM_XY_ZMP_ERR = 0;
				...
				SUM_X_ZMP_ERR += X_ZMP - pSharedMemory->xzmp_offset - Ref_ZMP[0] + 8.0*Ls/100;
	...
	// RLAR & RLAP 버츄얼 스프링 댐퍼 제어 //
	if(Walking_stage[LF_1] != 10 && OneKG_LOK_Flag == 1 && LandLOK_Flag == 0) {
 			Del_LAR_RLAR_TORQ_CON = 1.0*RLAR_TORQ_CON_BP(FTSensor[LFFT].Mx,1)*0.f;  
			Del_LAP_RLAP_TORQ_CON = -1.0*RLAP_TORQ_CON_BP(-FTSensor[LFFT].My,1)*0.f;
 			CNT_RLAR_TORQ_CON = 0.;
 			Old_Del_LAR_RLAR_TORQ_CON = Del_LAR_RLAR_TORQ_CON;
			Old_Del_LAP_RLAP_TORQ_CON = Del_LAP_RLAP_TORQ_CON;
			...
			Del_LAP_RLAP_TORQ_CON = Old_Del_LAP_RLAP_TORQ_CON - Old_Del_LAP_RLAP_TORQ_CON*0.5*(1-cos(PI*CNT_RLAR_TORQ_CON/40));
 			...
 			if (CNT_RLAR_TORQ_CON < 40 /*&& Old_Del_LAR_RLAR_TORQ_CON != 0.0*/) CNT_RLAR_TORQ_CON++;
 			else {Old_Del_LAR_RLAR_TORQ_CON = 0.; Del_LAR_RLAR_TORQ_CON = 0.; /*CNT_RLAR_TORQ_CON = 0;*/ RLAR_TORQ_CON_BP(0,0);
				  Old_Del_LAP_RLAP_TORQ_CON = 0.; Del_LAP_RLAP_TORQ_CON = 0.; RLAP_TORQ_CON_BP(0,0);}
	}

	if(Walking_stage[RF_1] != 10 && OneKG_ROK_Flag == 1 && LandROK_Flag == 0) {
			Del_RAR_RLAR_TORQ_CON = 1.0*RLAR_TORQ_CON_BP(FTSensor[RFFT].Mx,1)*0.f;
			Del_RAP_RLAP_TORQ_CON = -1.0*RLAP_TORQ_CON_BP(-FTSensor[RFFT].My,1)*0.f;
 			CNT_RLAR_TORQ_CON = 0.;
 			Old_Del_RAR_RLAR_TORQ_CON = Del_RAR_RLAR_TORQ_CON;
			Old_Del_RAP_RLAP_TORQ_CON = Del_RAP_RLAP_TORQ_CON;
	}
			...
			Del_RAP_RLAP_TORQ_CON = Old_Del_RAP_RLAP_TORQ_CON - Old_Del_RAP_RLAP_TORQ_CON*0.5*(1-cos(PI*CNT_RLAR_TORQ_CON/40));
 			...
 			if (CNT_RLAR_TORQ_CON < 40 /*&& Old_Del_RAR_RLAR_TORQ_CON != 0.0*/) CNT_RLAR_TORQ_CON++;
 			else {Old_Del_RAR_RLAR_TORQ_CON = 0.; Del_RAR_RLAR_TORQ_CON = 0.; /*CNT_RLAR_TORQ_CON = 0;*/ RLAR_TORQ_CON_BP(0,0);
				  Old_Del_RAP_RLAP_TORQ_CON = 0.; Del_RAP_RLAP_TORQ_CON = 0.; RLAP_TORQ_CON_BP(0,0);}
	...
	Del_RLFZ_Posture_Walking =  1.*TR_INT_CON_WALK(0.,1.0*IMUSensor[CENTERIMU].Roll,1);    // 가속도계 + ZMP 에 의한 경사만으로 상체 롤 기울기 교정 //
   ...
   if(Early_Landing_Flag[3] == 1 && temp_X[3] == 0. && temp_Y[3] == 0. && temp_th[3] == 0.) {temp_X[3] = Ref_L1[0]; temp_Y[3] = Ref_L1[1]; temp_th[3] = Ref_th[3]; EL_Del_xy_CNT[3] = 0;}
	...
	BC_Y_CON = - Del_PC_Y_DSP_YZMP_CON*1. - 1.*Del_PC_Y_SSP_YZMP_CON - 1.*Ref_CON_SUM_Y_ZMP_ERR;  // update
	BC_X_CON = - Del_PC_X_DSP_XZMP_CON*1. - 1.*Del_PC_X_SSP_XZMP_CON - 1.*Ref_CON_SUM_X_ZMP_ERR;  // update
	...
	_online_pattern.offset_LAR = (Del_LAR_Init + Del_LAR_RLAR_TORQ_CON + Del_RLAR_Posture_Walking*env_upright + 0.0*RHR_Compen_Ang)*D2R;
	_online_pattern.offset_LAP = (Del_LAP_RLAP_TORQ_CON + Del_RLHP_Init + HeelToe_LAP*ftemp)*D2R;

    _online_pattern.offset_RAR = (Del_RAR_Init + Del_RAR_RLAR_TORQ_CON + Del_RLAR_Posture_Walking*env_upright + 0.0*LHR_Compen_Ang)*D2R;
	_online_pattern.offset_RAP = (Del_RAP_Init + Del_RAP_RLAP_TORQ_CON + Del_RLHP_Init + HeelToe_RAP*ftemp)*D2R;

	_online_pattern.offset_RHR = (Del_RHR_VIB_CON*env_vib + RHR_Compen_Ang)*D2R;
	_online_pattern.offset_LHR = (Del_LHR_VIB_CON*env_vib + LHR_Compen_Ang + RF_Del_Y_Outside)*D2R;

	_online_pattern.offset_RHP = (Del_RHP_VIB_CON*env_vib + Del_RLHP_Posture_Walking*env_upright)*D2R;
	_online_pattern.offset_LHP = (Del_LHP_VIB_CON*env_vib + Del_RLHP_Posture_Walking*env_upright)*D2R;
	...
	_log_temp[29] = LandLOK_Flag;
	_log_temp[30] = LandROK_Flag;
	...
	_log_temp[36] = FTSensor[RFFT].Fz;
	_log_temp[37] = FTSensor[LFFT].Fz;
	_log_temp[38] = Walking_stage[RF_1];
	_log_temp[39] = Walking_stage[LF_1];

	/*
		_log_temp[40] = _Q_34x1[RAP];
		_log_temp[41] = _Q_34x1[RAR];
		_log_temp[42] = meas_Q_34x1[RAP];
		_log_temp[43] = meas_Q_34x1[RAR];
	...
	*/
	...
	_log_temp[55] = land_ok_cnt_RF;
	_log_temp[56] = land_ok_cnt_LF;
	_log_temp[57] = OneKG_ROK_Flag;
	_log_temp[58] = OneKG_LOK_Flag;
